...
async function splitPDF() {
  const file = document.getElementById('pdfFilesSplit')?.files[0];
  if (!file) {
    showToast('Select a PDF file to split', 'error');
    return;
  }
  ...
  const percentage = document.getElementById('splitPercentage');
  const status = document.getElementById('splitStatus');
  const statusIcon = document.getElementById('splitStatusIcon');
  ...
  try {
    ...
    if (pageCount === 1) {
      showToast('This PDF only has one page — nothing to split.', 'info');
      statusIcon.textContent = '⚠️';
      statusText.textContent = 'Single-page PDF — no split needed.';
      progressContainer.style.display = 'none';
      return;
    }
    ...
    for (let i = 0; i < pageCount; i++) {
      ...
      progressFill.style.width = `${progress}%`;
      percentage.textContent = `${Math.round(progress)}%`;
      statusText.textContent = `Splitting page ${i + 1} of ${pageCount}...`;
    ...
    statusIcon.textContent = '✅';
    statusText.textContent = `Split complete! ${results.length} pages created.`;

    // Show results
    resultsList.innerHTML = `
      <div class="result-item" style="background: linear-gradient(135deg, #e8f5e8, #f0fff0); border-left: 5px solid #28a745;">
        <div>
          <strong>✅ Split completed successfully!</strong>
          <div style="color:#666;font-size:0.9rem;margin-top:0.5rem;">
            Created ${results.length} individual page PDFs
          </div>
        </div>
      </div>
    `;
    ...
    for (const fileObj of results) {
      ...
      item.innerHTML = `
        <div class="result-info">
          <div class="result-name">${fileObj.name}</div>
          <div class="result-size">Size: ${(fileObj.blob.size / 1024).toFixed(1)} KB</div>
        </div>
        <a href="${url}" download="${fileObj.name}" class="tool-btn tool-btn-primary">Download</a>
      `;
    ...
    showToast('Split complete — download your pages below!', 'success');

  } catch (err) {
    console.error('Split error:', err);
    status.className = 'file-status';
    statusIcon.textContent = '❌';
    statusText.textContent = 'Split failed: ' + err.message;
...
function setupHandlers() {
  document.getElementById('mergeDropZone').addEventListener('click', () => {
    document.getElementById('pdfFiles').click();
  });
  document.getElementById('splitDropZone').addEventListener('click', () => {
    document.getElementById('pdfFilesSplit').click();
  });
